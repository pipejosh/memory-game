...
The how to play form (the form that is used as a tutorial)
*/
...
{
    // Initialize all necesary variables
    ...
    {
        // For the exit one we call the exit action
        exitAction();
    }

    private void btnResetWonActionPerformed(java.awt.event.ActionEvent evt) 
    {
        // For the reset win button we call the reset action
        resetAction();
    }

    private void btnResetLoseActionPerformed(java.awt.event.ActionEvent evt) 
    {
        // For the reset lose button we call the reset action
        resetAction();
    }

    // The buttons in game actions
    private void btn6ActionPerformed(java.awt.event.ActionEvent evt) 
    {
        // We call the method checkAndUpdate from Scripts.java (more of that in Scripts.java)
        scriptsForOtherButtons.checkAndUpdate(btn6, new Runnable() 
        {
            // Once the action if finnished we call a runable, a action at the end
            @Override
            public void run()     
            {
                // We call our funciont actionDoOnFinish on the end of the action
    ...
    {
        // We call the method areBothBueeonsSelected from Help.java (more of that later)
    ...
    // This function sets all components to their default state (for the form)
    public void setComponenetsBeginning()
    {
        // Set certain components visibility to false
        ...
        activateLoseButtons();
        // Change the sate of all the toggle buttons to false
    ...
    {
        // Create 2 buttons Array (very usefull) + create 2 instance of scripts 1 for each array of buttons(the whole brain of the game)
    ...
    // This method takes as a parameter the current button, a button pair, and does an action at the end
    public void areBothButtonsSelected(JToggleButton currentButton, JToggleButton pairButton, Runnable onFinish)
    {
        // If the current buttons is selected and its pair is also selecter
        if (currentButton.isSelected() && pairButton.isSelected())
        {
            // We run the runnable (an action at the end)
            onFinish.run(); 
        }
    }

    // This method takes as a paramenter a Jpanel and makes it visible
    public void setVisibleComponents(JPanel panel)
    {
        panel.setVisible(true);
    }

    // This method takes as a paramenter a Jpanel and makes it non visible
    public void setNonVisibleComponents(JPanel panel)
    {
        panel.setVisible(false);
    }

    // This method takes as a parameter a option (a string) and in base of that string makes differents accctions
    public void actionDoOnFinish(String option)
    {
        switch (option)
        {
            // If the string is first buttons
            case "first buttons" ->
            {
                // Set 2 panels to visible
                setVisibleComponents(pnlFirstButtons);
                setVisibleComponents(pnlSecondButtons);
            }

            // If the string is win buttons
            case "win buttons" ->
            {
                // Deactivate lose buttons and set one panel to be visible
                deactivateLoseButtons();
                setVisibleComponents(pnlResetWin);
            }
        
            // If the string is lose buttons
            case "lose buttons" ->
            {
                // Deactivate lose buttons and other buttons and set the reset panel to be visible
                deactivateLoseButtons();
            ...
            default ->
            {
                // We let the programmer know that, sending this message to the terminal
                System.out.println("PLEASE ENTER A VALID OPTION"); 
            }
        }
    }

    // This method activates lose buttons
    public void activateLoseButtons()
    {
        // Activate button 3 and 4
        btn3.setEnabled(true);
        btn4.setEnabled(true);
    }

    // This method deactivate lose buttons
    public void deactivateLoseButtons()
    {
        // Deactivate button 3 and 4
        btn3.setEnabled(false);
        btn4.setEnabled(false);
    }

    // This method is done when the reset button is pressed
    public void resetAction()
    {
        // Reestart the components + acivate buttons from Scripts.java
        setComponenetsBeginning();
        scripts.activateButtons();
    }

    // This method is done when exit button is pressed
    public void exitAction()
    {
        // Create an instance of Main menu
        MainMenu menu = new MainMenu();

        // Stop the 2 songs (because we create 2 instances of scripts therefore now we have to stop it twice)
        scriptsForOtherButtons.musicPlayer.stopSong();
        scripts.musicPlayer.stopSong();

        // Close the current window (the help one) + set visible the menu
        this.dispose();
